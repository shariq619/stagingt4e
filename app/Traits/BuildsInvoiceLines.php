...
trait BuildsInvoiceLines
{
    protected function buildInvoiceAndLine($detail, $user, $invoice = null, $isNormal = false)
    {
        ...
        $invoice->lines()->create([
            ...
            'product_description' => $desc ?? $detail->course_name ?? $detail->course?->name ?? 'Course',
        ...
        if (!$isNormal && empty($detail->invoice_pdf_url)) {
            $filePath = "crm/training-courses/invoices/{$invoice->invoice_no}.pdf";
            Storage::disk('public')->makeDirectory('crm/training-courses/invoices');
            if (Storage::disk('public')->exists($filePath)) {
                Storage::disk('public')->delete($filePath);
            }
            $delegate = $user;
            $costing = $detail;
            $pdf = Pdf::loadView('crm.pdfs.training_course', [
                'delegate' => $delegate,
                'costing' => $costing,
            ])->setPaper('a4');
            Storage::disk('public')->put($filePath, $pdf->output());
            $detail->invoice_pdf_url = $filePath;
