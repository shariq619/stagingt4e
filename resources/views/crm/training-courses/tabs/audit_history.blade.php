...
@push('css')
    ...
    <style>
        ...
        .row2 .right input[type="checkbox"] {
            width: 18px;
            height: 18px;
            border-radius: 4px;
    ...
    <div class="card">
        <div class="toolbar">
            <div class="grid">
                ...
                <div class="cell x5">
                    ...
                    <input class="input" id="f-q"
                           placeholder="Invoice no, ID, user, IP, any text — even ‘From: … → To: …’">
                </div>
                <div class="cell x1">
            ...
            <div class="row2">
                ...
                <div class="right">
                    <span class="label">Reassign only</span>
                    <input type="checkbox" id="f-reassign" style="height:20px;width:20px">
                </div>
            </div>
...
@push('js')
    ...
    <script>
        $(function () {
            ...
            const url = @json(route('crm.training-courses.audit.dt',['cohort'=>'__ID__'])).
            replace('__ID__', cohortId);
            ...
            const table = $('#auditTable').DataTable({
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.from = $('#f-from').val();
                        d.to = $('#f-to').val();
                        d.reassign = $('#f-reassign').is(':checked') ? '1' : '';
                ...
                columns: [
                    ...
                    {
                        data: 'event_badge',
                        name: 'payment_audit_logs.event',
                        orderable: false,
                        searchable: false,
                        width: '260px',
                        render: function (data, type, row) {
                            const re = row.is_reassignment ? '<span class="badge b-re">Reassignment</span>' + (row.reassign_recap || '') : '';
                            return data + ' ' + re;
                        }
                    },
                    {data: 'type', name: 'payment_audit_logs.auditable_type', width: '150px'},
                    {
                        data: 'auditable_id',
                        name: 'payment_audit_logs.auditable_id',
                        width: '140px',
                        orderable: false,
                        searchable: false
                    },
                    {data: 'user_name', name: 'users.name', width: '180px'},
                    {data: 'ip', name: 'payment_audit_logs.ip', width: '140px'},
            ...
            $('#f-from,#f-to,#f-reassign').on('change', reload);
            ...
            $(document).on('click', '.btn-changes', function () {
                ...
                $meta.text('#' + (row.id || '') + ' • ' + (row.type || '') + ' • ' + ((row.auditable_invoice_no || row.auditable_id) || '') + (row.at ? ' • ' + row.at : ''));
                $bd.html(html);
