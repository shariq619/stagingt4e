        ...
        <div class="row">
            <div class="col-md-12">
                <div class="row headerDetail">
                    <div class="col-12">
                        <img height="200" width="200" alt="Logo" class="img-fluid"
                             src="{{ asset('frontend/img/T4E-logo_Full-Colour-e1611494943115.png') }}" />
    ...
    <div class="formWrapper mb-5">
        ...
        <form method="POST" action="{{ route('backend.risk.assessment.store') }}" id="riskForm">
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <input type="text" class="form-control" name="course"
                           value="{{ $cohort->course->name ?? "" }}" readonly>
                </div>
            ...
            <div class="form-group">
                <label>Tutor(s):</label>
                <input type="text" class="form-control" name="tutors"
                       value="{{ $cohort->trainer->name ?? "" }}" readonly>
            </div>

            <div class="form-group">
                <label>Training Venue:</label>
                <input type="text" class="form-control" name="training_venue"
                       value="{{ $cohort->venue->venue_name ?? "" }}" readonly>
            </div>
                    ...
                    @foreach($checklist as $key => $item)
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="radio"
                                       name="checklist[{{ $key }}][status]"
                                       value="safe"
                                    {{ old("checklist.$key.status", data_get($riskAssessment->checklist, "$key.status")) == 'safe' ? 'checked' : '' }}>
                            </td>
                            <td class="text-center">
                                <input type="radio"
                                       name="checklist[{{ $key }}][status]"
                                       value="unsafe"
                                    {{ old("checklist.$key.status", data_get($riskAssessment->checklist, "$key.status")) == 'unsafe' ? 'checked' : '' }}>
                            </td>
                            <td>
                                <input type="text"
                                       class="form-control"
                                       name="checklist[{{ $key }}][comment]"
                                       value="{{ old("checklist.$key.comment", data_get($riskAssessment->checklist, "$key.comment")) }}">
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" rows="3" name="hazards">{{ $riskAssessment->hazards }}</textarea>
            </div>

            <!-- Control measures -->
            <div class="form-group">
                <label>Implemented control measures/briefing to delegates:</label>
                <textarea class="form-control" rows="3" name="control_measures">{{ $riskAssessment->control_measures }}</textarea>
            </div>
            ...
            <div class="form-row">
                <div class="form-group col-md-8">
                    ...
                    <div id="signature-pad" class="signature-pad">
                        <img src="{{ $riskAssessment->tutor_signature }}" alt="Learner's Signature"
